import { IEventPublisher } from '../ports/IEventPublisher'
import { IAIResponseRepository } from '../ports/IAIResponseRepository'
import { v4 as uuidv4 } from 'uuid'

export interface AIResponsePayload {
  userId: string
  senderId: string
  messageId: string
  conversationId?: string
  aiResponse: string
  confidence?: number
  model?: string
  processingTime?: number
}

export interface ChunkSendFunction {
  (recipient: string, message: string, messageId: string): Promise<string>
}

export interface ChunkResult {
  success: boolean
  externalMessageId?: string
  channel?: string
  error?: string
}

const ROUTING_KEYS = {
  INSTAGRAM_AI_RESPONSE_CHUNK_FAILED: 'channels.instagram.ai-response-chunk-failed',
  INSTAGRAM_AI_RESPONSE_DLQ: 'channels.instagram.ai-response-dlq',
}

export class HandleAIResponseUseCase {
  private readonly MAX_CHUNK_SIZE = 4096
  private readonly MAX_RETRIES = 3

  constructor(
    private readonly eventBus: IEventPublisher,
    private readonly aiResponseRepo: IAIResponseRepository,
  ) {}

  async execute(
    payload: AIResponsePayload,
    sendFn: ChunkSendFunction,
  ): Promise<void> {
    const validAiResponse = payload.aiResponse || 'No AI response generated'

    const aiResponseRecord = await this.aiResponseRepo.create({
      id: uuidv4(),
      userId: payload.userId,
      senderId: payload.senderId,
      messageId: payload.messageId,
      originalMessage: '',
      aiResponse: validAiResponse,
      model: payload.model ?? 'unknown',
      confidence: payload.confidence ?? 0,
      processingTime: payload.processingTime ?? 0,
    })

    const chunks = this.splitMessageIntoChunks(validAiResponse)
    if (chunks.length === 0) {
      this.eventBus.publish(ROUTING_KEYS.INSTAGRAM_AI_RESPONSE_DLQ, {
        aiResponseId: aiResponseRecord.id,
        userId: payload.userId,
        senderId: payload.senderId,
        reason: 'AI response is empty',
        timestamp: Date.now(),
      })
      return
    }

    const chunkRecords = await this.aiResponseRepo.createChunks(
      chunks.map((content, index) => ({
        id: `${aiResponseRecord.id}_chunk_${index}`,
        aiResponseId: aiResponseRecord.id,
        chunkNumber: index + 1,
        content,
      })),
    )

    let sentCount = 0
    for (const chunk of chunkRecords) {
      const result = await this.sendChunkWithRetry(
        chunk,
        payload.senderId,
        sendFn,
      )

      if (result.success) {
        await this.aiResponseRepo.updateChunkStatus(chunk.id, 'SENT', {
          externalMessageId: result.externalMessageId,
          channel: 'instagram',
          sentAt: new Date(),
        })
        sentCount++
      } else {
        await this.aiResponseRepo.updateChunkStatus(chunk.id, 'FAILED')
        this.eventBus.publish(ROUTING_KEYS.INSTAGRAM_AI_RESPONSE_CHUNK_FAILED, {
          chunkId: chunk.id,
          aiResponseId: aiResponseRecord.id,
          senderId: payload.senderId,
          error: result.error,
        })
      }
    }

    const totalChunks = chunkRecords.length
    const status = sentCount === totalChunks ? 'SENT' : sentCount > 0 ? 'PARTIAL' : 'FAILED' as const
    await this.aiResponseRepo.updateStatus(aiResponseRecord.id, status, { sentChunks: sentCount })
  }

  async handleFailedChunk(
    chunkId: string,
    aiResponseId: string,
    senderId: string,
    sendFn: ChunkSendFunction,
    retryCount: number,
  ): Promise<void> {
    const newRetryCount = retryCount + 1

    if (newRetryCount < this.MAX_RETRIES) {
      await this.aiResponseRepo.updateChunkStatus(chunkId, 'PENDING', { retryCount: newRetryCount })
      this.eventBus.publish(ROUTING_KEYS.INSTAGRAM_AI_RESPONSE_CHUNK_FAILED, {
        chunkId,
        aiResponseId,
        senderId,
        retryCount: newRetryCount,
      })
    } else {
      await this.aiResponseRepo.updateChunkStatus(chunkId, 'FAILED')
      await this.sendToDLQ(aiResponseId, senderId, `Chunk ${chunkId} failed after ${this.MAX_RETRIES} retries`)
    }
  }

  async sendToDLQ(
    aiResponseId: string,
    senderId: string,
    reason: string,
  ): Promise<void> {
    this.eventBus.publish(ROUTING_KEYS.INSTAGRAM_AI_RESPONSE_DLQ, {
      aiResponseId,
      senderId,
      reason,
      timestamp: Date.now(),
    })
  }

  splitMessageIntoChunks(message: string): string[] {
    if (!message || message.length === 0) {
      return []
    }

    const chunks: string[] = []
    let remainingText = message

    while (remainingText.length > 0) {
      chunks.push(remainingText.substring(0, this.MAX_CHUNK_SIZE))
      remainingText = remainingText.substring(this.MAX_CHUNK_SIZE)
    }

    if (chunks.length === 1) {
      return chunks
    }

    return chunks.map((chunk, index) => `[${index + 1}/${chunks.length}] ${chunk}`)
  }

  private async sendChunkWithRetry(
    chunk: { id: string; content: string },
    senderId: string,
    sendFn: ChunkSendFunction,
  ): Promise<ChunkResult> {
    let lastError: Error | null = null

    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        const externalMessageId = await sendFn(
          senderId,
          chunk.content,
          `chunk_${chunk.id}_attempt_${attempt}`,
        )

        return {
          success: true,
          externalMessageId,
          channel: 'instagram',
        }
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(String(error))

        if (attempt < this.MAX_RETRIES) {
          await this.sleep(Math.pow(2, attempt - 1) * 1000)
        }
      }
    }

    return {
      success: false,
      error: lastError?.message || 'Unknown error',
    }
  }

  private sleep(ms: number): Promise<void> {
    return new Promise((resolve) => setTimeout(resolve, ms))
  }
}
